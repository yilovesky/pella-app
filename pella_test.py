...
def run_test():
    ...
    with SB(uc=True, xvfb=True) as sb:
        ...
        try:
            ...
            send_tg_notification("进度日志 📸", "已提交邮箱，等待发送验证码", "step2_input_email.png")
            sb.sleep(5)
            ...
            logger.info(f"🔗 [面板监控] 识别到续期网址，准备物理点击...")
            ...
            try:
                ...
                if sb.is_element_visible(cf_iframe):
                    ...
                    send_tg_notification("进度日志 📸", "已完成 Kata 穿透", "step6_after_cf.png")
                else:
                    sb.uc_gui_click_captcha()
            except: pass
            ...
            send_tg_notification("进度日志 📸", "倒计时结束，点击最终按钮", "step8_wait_timer.png")
            ...
            # 点完 GO 之后的操作：切回原标签页，等待并刷新确认
            if click_final:
                sb.driver.switch_to.window(main_window)
                logger.info("⌛ [面板监控] 原标签页原地等待 15 秒...")
                sb.sleep(15)
                for r in range(3):
                    sb.sleep(5)
                    logger.info(f"🔄 [面板监控] 正在执行第 {r+1} 次刷新确认...")
                    sb.refresh_page()
            ...
            if click_final:
                ...
                send_tg_notification("操作反馈 ⚠️", f"执行完毕。续期前: {expiry_before}\n当前时间: {expiry_after}", "final_result.png")
